        ...
        <div class="row">
            <div class="col-12 col-xl d-flex align-items-center mb-3 mb-xl-0">
                <div>
                    ...
                    <p class="text-muted"><?= l('link_statistics.statistics.country_help') ?></p>
                </div>
            </div>
                ...
                <div class="d-flex justify-content-between mb-1">
                    <div class="text-truncate">
                        ...
                        <?php if($row->country_code): ?>
                            <a href="<?= url('link-statistics/' . $data->link->link_id . '?type=city_name&country_code=' . $row->country_code . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>" title="<?= $row->country_code ?>" class="align-middle"><?= $row->country_code ? get_country_from_country_code($row->country_code) : l('link_statistics.statistics.country_unknown') ?></a>
                        <?php else: ?>
                            <span class="align-middle"><?= $row->country_code ? get_country_from_country_code($row->country_code) : l('link_statistics.statistics.country_unknown') ?></span>
                        <?php endif ?>
                    </div>
